...
from .dataset import ImageFolderInstance, CXRDataset, CXRContrastDataset, CXRContrastClassDataset
...
def build_contrast_loader(opt, ngpus_per_node):
    ...
    #FOLDS = [1,2,3,4,5,6]
    FOLDS = [1,2,3,4,5]
    train_dataset = CXRContrastClassDataset(data_folder, FOLDS)
...
def build_linear_loader(opt, ngpus_per_node):
    ...
    if custom:
        ...
        train_dataset = CXRDataset(opt.data_folder, FOLDS, do_transform=True, pathology=True)
        val_dataset = CXRDataset(opt.data_folder, ['TEST'], do_transform=False, pathology=True, test=True)
