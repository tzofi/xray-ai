...
class RandomCrop(object):
    ...
    def __call__(self, img):
        ...
        if self.pad_if_needed and img.shape[1] < self.size[1]:
            img = pad(img, (int((1 + self.size[1] - img.shape[1]) / 2), 0))
        # pad the height if needed
        if self.pad_if_needed and img.shape[0] < self.size[0]:
            img = pad(img, (0, int((1 + self.size[0] - img.shape[0]) / 2)))
...
class RandomPerspective(object):
    ...
    def __call__(self, img):
        ...
        return perspective(img, *ret, resample=self.resample, fillcolor=self.fillcolor)
...
class RandomAffine6(object):
    ...
    def __call__(self, img):
        ...
        return affine6(img, *ret, resample=self.resample, fillcolor=self.fillcolor)
...
class RandomSPNoise(object):
    ...
    def __call__(self, img):
        ...
        """
        if random.random() < self.p:
            return salt_and_pepper(img, self.prob)
        return img
