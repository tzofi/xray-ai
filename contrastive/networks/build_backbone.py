...
class RGBSingleHead(nn.Module):
    ...
    def __init__(self, name='resnet50', head='linear', feat_dim=128, channels=3):
        ...
        self.encoder = model_dict[name](width=width, in_channel=channels)
...
def build_model(opt):
    ...
    model = NAME_TO_FUNC[model_key](opt.arch, opt.head, opt.feat_dim, opt.channels)
    if opt.mem == 'moco':
        model_ema = NAME_TO_FUNC[model_key](opt.arch, opt.head, opt.feat_dim, opt.channels)
    else:
        model_ema = None
