...
class ResNet_CIFAR(nn.Module):
    ...
    def __init__(self, block, layers, num_features=128, reparam=False, head='head'):
    ...
    def forward(self, x):
        ...
        elif self.head == 'double':
            return self.learning_head(h_out)
        else:
...
def resnet50_cifar(num_features=128, reparam=False, head='head'):
    return ResNet_CIFAR(Bottleneck_CIFAR, [3, 4, 6, 3], num_features, reparam, head)


def resnet18_cifar(num_features=128, reparam=False, head='head'):
    return ResNet_CIFAR(BasicBlock_CIFAR, [2, 2, 2, 2], num_features, reparam, head)


class SimpleNet(nn.Module):
    def __init__(self, input_dim=131072, num_classes=4):
